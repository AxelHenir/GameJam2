...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController, IPhysicsObject
    {
        ...
        private void RemoveTransientVelocity()
        {
            ...
            // Well, this is gross
            var requiresReduction = false;
            if (velocityBeforeReduction.x < 0 && _decayingTransientVelocity.x < velocityBeforeReduction.x) requiresReduction = true;
            else if (velocityBeforeReduction.x > 0 && _decayingTransientVelocity.x > velocityBeforeReduction.x) requiresReduction = true;
            else if (velocityBeforeReduction.y < 0 && _decayingTransientVelocity.y < velocityBeforeReduction.y) requiresReduction = true;
            else if (velocityBeforeReduction.y > 0 && _decayingTransientVelocity.y > velocityBeforeReduction.y) requiresReduction = true;

            var decay = Stats.Friction * Stats.AirFrictionMultiplier * Stats.ExternalVelocityDecayRate;
            _decayingTransientVelocity = Vector2.MoveTowards(_decayingTransientVelocity, Vector2.zero, decay * (requiresReduction ? 5 : 1) * _delta);
        }
        ...
        private const float RAY_INSET = 0.1f;
        ...
        private void CalculateCollisions()
        {
            ...
            {
                _groundHit = Physics2D.Raycast(point, -Up, GrounderLength + _currentStepDownLength, ~Stats.PlayerLayer);
        ...
        {
            var extent = _character.StandingColliderSize.x / 2 - RAY_INSET;
        ...
        private bool IsStandingPosClear(Vector2 pos) => CheckPos(pos, _character.StandingColliderSize);
        ...
        private bool CheckPos(Vector2 pos, Vector2 size)
        {
            ...
            var hit = Physics2D.OverlapBox(pos, size, 0, ~Stats.PlayerLayer);
        ...
        private void OnTriggerExit2D(Collider2D other)
        {
            ...
            else if (other.TryGetComponent(out IPhysicsMover mover))
            {
                _activatedMovers.Remove(mover);
            }
        }
